...
impl TryFrom<RawCommand> for Command {
    ...
    fn try_from(row: RawCommand) -> Result<Self, Self::Error> {
...
///
/// Rows that cannot be deserialized or converted to a known command type are
...
pub fn from_reader(reader: impl std::io::Read) -> impl Iterator<Item = Command> {
    csv::ReaderBuilder::new()
        ...
        .from_reader(reader)
        .into_deserialize()
        .filter_map(|r: Result<RawCommand, _>| r.ok())
...
mod test {
    ...
    fn test_parse_deposit() {
        let commands = parse("type,client,tx,amount\ndeposit,1,1,1.5");
        assert!(matches!(
            commands[0],
            Command::Deposit {
                client: 1,
        ...
        assert_eq!(
            if let Command::Deposit { amount, .. } = commands[0] {
                amount
            } else {
                unreachable!()
            },
    ...
    fn test_parse_withdrawal() {
        ...
        assert_eq!(
            if let Command::Withdrawal { amount, .. } = commands[0] {
                amount
            } else {
                unreachable!()
            },
    ...
    fn test_parse_dispute() {
        let commands = parse("type,client,tx,amount\ndispute,1,1,");
        assert!(matches!(commands[0], Command::Dispute { client: 1, tx: 1 }));
    }

    #[test]
    fn test_parse_resolve() {
        let commands = parse("type,client,tx,amount\nresolve,1,1,");
        assert!(matches!(commands[0], Command::Resolve { client: 1, tx: 1 }));
    }
    ...
    fn test_invalid_rows_are_skipped() {
        ...
        assert!(matches!(commands[0], Command::Deposit { tx: 2, .. }));
    }
    ...
    fn test_large_amount_does_not_flip_sign() {
        ...
        let amount = if let Command::Deposit { amount, .. } = commands[0] {
            amount
        } else {
            unreachable!()
        };
